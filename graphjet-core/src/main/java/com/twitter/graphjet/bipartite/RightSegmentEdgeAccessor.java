...
/**
 * This wraps access to edges in a {@link BipartiteGraphSegment} using a common interface in the
 * {@link SegmentEdgeAccessor}
 */
public class RightSegmentEdgeAccessor extends SegmentEdgeAccessor<BipartiteGraphSegment> {
   ...
   *                                            regular long id
   */
  public RightSegmentEdgeAccessor(
  ...
  public EdgeIterator getNodeEdges(int segmentId, long node) {
    return readerAccessibleInfo.segments.get(segmentId).getRightNodeEdges(
        node,
  ...
  public void rebuildIterators(int oldestSegmentId, int liveSegmentId) {
    ...
    for (int i = oldestSegmentId; i <= liveSegmentId; i++) {
      ...
      segmentNodeIntIteratorMap.put(i, segment.initializeRightNodeEdgesIntIterator());
    }
  }
}
